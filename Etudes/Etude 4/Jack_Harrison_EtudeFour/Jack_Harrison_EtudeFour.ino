...
void setRGB()
{
  switch (mode)
  {
    ...
    case 4:
      {
        analogWrite(LED_PIN_R, 100);   // Turn on the LED -R
        analogWrite(LED_PIN_G, 0);   // Turn on the LED -G
        analogWrite(LED_PIN_B, 200);   // Turn on the LED -B
        break;
      }
  }
}
...
/**************************************************************************
 ...
 1.
 The shift register is a integrated logic circuit that is used to store or transfer data in binary. The 
 device loads the data in its inputs and shifts it to its output once every clock cycle.

 2.
 By adding another button we could eliminate the need to rerun through the modes. The button would
 simply reload Mode 4.

 3.
 I'm not sure how I would store this frequency change. Sorry :(

 4.
 a) Similar to Tim, Owen and I's first assignment, implementing a change to a final song. You could 
 add an accelorometer that changes the  tempo of the current song you are on. It would take the X-value
 from said accelorometer and use this to multiply with the time between each stored songs' notes. This 
 allows you to remix the song even after storing it. this mode could also be toggled to ensure that you 
 are at first, hearing what you intended to record. This toggle would als obe useful in calibration as 
 everytime it was turned on, it could recalibrate the X-sensor.

 b) Another neat idea for the synth would be to add visualization. By adding both LEDs and perhaps a 
 small LCD screen. I could create compelling visual accompanyment with whatever tone comes out of the
 device. You could also add additional potentiometers to alter the visuals coming out of the device,
 changing delays and patterns to create the desired effect. Expanding on this one could create the 
 ultimate entertainment device, a combined audio/visual synthesizer. I think people forget about live
 visuals all the time, and they could make a huge difference in the world of live DJs.
