...
void setRGB()
{
  switch (mode)
  {
    ...
    case 4:
      {
        analogWrite(LED_PIN_R, 0);   // Turn on the LED -R
        analogWrite(LED_PIN_G, 150);   // Turn on the LED -G
...
void live()
{
  //implemented my code from etude 2, added the offset for theramin
  offsetFrequency = getPhotoFrequency();
  // output sound
  if (analogRead(NOTE_IN_PIN) > 0) {
    tone(BUZZER_PIN, (analogRead(NOTE_IN_PIN) + offsetFrequency * 7) / 6, duration);
    delay(50);
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES)
  {
    ...
    */
    ...
    {
      noTone(BUZZER_PIN);
      // Wrote the code to turn OFF the buzzer
    }
    /*** STATE C::: *******************************************
...
void startUpTimer()
{
  //started timer and reset notButton boolean
  startTime = millis();
...
void updateTimer()
{
  //function to update the timer everytime a button is pressed so it startes from 0
...
void playCurrentNote()
{
  ...
  //function to call in various other functions, simply plays the note and records it to use in array
  tone(BUZZER_PIN, (testNote + offsetFrequency * 7) / 6, 100);
  //  delay(100);
...
{
  //fill arrays with notes and with the duration said notes were played
...
{
  //simply reads the pin connected to the theramin
...
{
  //running average code mostly taken from sabine's slides, Thanks sabine!
...
{
  //I adjust the R value as my LED is a bit wonky
...
{
  //plays recorded notes in array using second durations array to govern duration. This is also where we use the LED color changing function
  for (int i = 0; i < 16; i ++) {
